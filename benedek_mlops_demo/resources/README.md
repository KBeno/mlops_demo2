...
The databricks CLI bundles top level is defined by file `benedek_mlops_demo/databricks.yml`.
...
ML Resource Configurations in this directory:
 - model workflow (`benedek_mlops_demo/resources/model-workflow-resource.yml`)
 - batch inference workflow (`benedek_mlops_demo/resources/batch-inference-workflow-resource.yml`)
 - monitoring resource and workflow (`benedek_mlops_demo/resources/monitoring-resource.yml`)
 - feature engineering workflow (`benedek_mlops_demo/resources/feature-engineering-workflow-resource.yml`)
 - model definition and experiment definition (`benedek_mlops_demo/resources/ml-artifacts-resource.yml`)
...
If you want to use different catalog names, please update the `targets` declared in the `benedek_mlops_demo/databricks.yml` and `benedek_mlops_demo/resources/ml-artifacts-resource.yml` files.
...
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|-------------------------------------|------------------------------------------------|
| dev         | The `dev` deployment target is used by ML engineers to deploy ML resources to development workspace with `dev` configs. The config is for ML project development purposes.                                                           | dev workspace        | dev-benedek_mlops_demo-model     | /dev-benedek_mlops_demo-experiment     |
| staging     | The `staging` deployment target is part of the CD pipeline. Latest main content will be deployed to staging workspace with `staging` config.                                                             | staging workspace    | staging-benedek_mlops_demo-model | /staging-benedek_mlops_demo-experiment |
| prod        | The `prod` deployment target is part of the CD pipeline. Latest release content will be deployed to prod workspace with `prod` config.                                                                      | prod workspace       | prod-benedek_mlops_demo-model    | /prod-benedek_mlops_demo-experiment    |
| test        | The `test` deployment target is part of the CI pipeline. For changes targeting the main branch, upon making a PR, an integration test will be triggered and ML resources deployed to the staging workspace defined under `test` deployment target. | staging workspace    | test-benedek_mlops_demo-model    | /test-benedek_mlops_demo-experiment    |
...
2. Complete the `TODO` in `benedek_mlops_demo/databricks.yml` to add the dev workspace URI under `targets.dev.workspace.host`.
...
Following this, update the batch_inference_job base parameters in `benedek_mlops_demo/resources/batch-inference-workflow-resource.yml` to pass
...
To get started, open `benedek_mlops_demo/resources/batch-inference-workflow-resource.yml`.  The file contains the ML resource definition of a batch inference job, like:
...
resources:
  jobs:
    batch_inference_job:
      name: ${bundle.target}-benedek_mlops_demo-batch-inference-job
      tasks:
...
The example above defines a Databricks job with name `${bundle.target}-benedek_mlops_demo-batch-inference-job`
that runs the notebook under `benedek_mlops_demo/deployment/batch_inference/notebooks/BatchInference.py` to regularly apply your ML model for batch inference. 
...
We specify a `batch_inference_job` under `resources/jobs` to define a databricks workflow with internal key `batch_inference_job` and job name `{bundle.target}-benedek_mlops_demo-batch-inference-job`.
...
`test-benedek_mlops_demo-batch-inference-job`. During the deployment of the `staging` environment config, the job name will be
`staging-benedek_mlops_demo-batch-inference-job`.
